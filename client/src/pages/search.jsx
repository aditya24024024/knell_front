...
const CATEGORIES = [
  ...
  "Musician", "Old Age Companion", "Online Meet", "Pet Companion", "Photography",
...
const dropdownStyle = {
  ...
  minWidth: 180, zIndex: 100, padding: "0.35rem 0",
};
...
const dropdownItemStyle = (active) => ({
  ...
  cursor: "pointer", display: "block", width: "100%", border: "none", textAlign: "left",
});
...
const FilterButton = ({ label, isActive, onClick }) => (
  <button onClick={onClick} style={{
    ...
    color: isActive ? "#5dc94a" : "#6b7a62", cursor: "pointer",
  }}>
    {label} {isActive ? "▲" : "▼"}
  </button>
);
      ...
      {(gigs.length > 0 || loading || activeFilterCount > 0) && (
        <div style={{ padding: "2rem 3rem" }}>
            ...
            <div style={{ position: "relative" }}>
              ...
              {openDropdown === "category" && (
                <div style={dropdownStyle}>
                  {CATEGORIES.map((cat) => (
                    <button key={cat} style={dropdownItemStyle(selectedCategory === cat)} onClick={() => { setSelectedCategory(cat); setOpenDropdown(null); }}>{cat}</button>
                  ))}
                </div>
              )}
            </div>
              ...
              {openDropdown === "budget" && (
                <div style={dropdownStyle}>
                  {BUDGET_OPTIONS.map((opt) => (
                    <button key={opt.label} style={dropdownItemStyle(selectedBudget.label === opt.label)} onClick={() => { setSelectedBudget(opt); setOpenDropdown(null); }}>{opt.label}</button>
                  ))}
                </div>
              )}
            </div>
              ...
              {openDropdown === "delivery" && (
                <div style={dropdownStyle}>
                  {DELIVERY_OPTIONS.map((opt) => (
                    <button key={opt.label} style={dropdownItemStyle(selectedDelivery.label === opt.label)} onClick={() => { setSelectedDelivery(opt); setOpenDropdown(null); }}>{opt.label}</button>
                  ))}
                </div>
              )}
            </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: "1.5rem", marginBottom: "2rem" }}>
            {gigs.map((gig) => <SearchGridItem gig={gig} key={gig.id} />)}
          </div>
          ...
          {loading && (
            <div style={{ display: "flex", justifyContent: "center", padding: "2rem 0" }}>
              <div style={{ width: 32, height: 32, border: "3px solid rgba(93,201,74,0.2)", borderTop: "3px solid #5dc94a", borderRadius: "50%", animation: "knell-spin 0.8s linear infinite" }} />
      ...
      {showEmptyState && (
        <div style={{ padding: "2rem 3rem" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "4rem 0 3rem" }}>
            ...
            <p style={{ fontFamily: "Space Mono, monospace", fontSize: "0.65rem", letterSpacing: "0.15em", color: "#3d4438", textTransform: "uppercase", margin: "0.75rem 0 0" }}>
          ...
          {featuredGigs.length > 0 && (
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", marginBottom: "1.5rem" }}>
                ...
                <div style={{ flex: 1, height: 1, background: "linear-gradient(to right, #3a8a2c, transparent)" }} />
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: "1.5rem" }}>
                {featuredGigs.map((gig) => <SearchGridItem gig={gig} key={gig.id} />)}
              </div>
            </div>
          )}
        </div>
